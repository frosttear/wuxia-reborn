...
const UI = {
    ...
    showScrollingCredits(items) {
        return new Promise(resolve => {
            ...
            let rafId = null, startTs = null, finished = false;
    ...
    showRebirthScreen(summary, availableTalents, cause) {
        ...
        const causeText = cause === 'boss' ? '败于天魔'
            ...
            : cause === 'true_final_boss' ? '困于轮回，败于最终审判'
        ...
        const rebirthNarrative = cause === 'boss'
            ...
            : cause === 'true_final_boss'
            ? '沈玄清收起那柄无形之剑，俯视着倒下的你。「你已走到了前所未有的终点，」他平静地说，「但终究，还未到时候。」黑暗中，胸口的双鱼玉佩发出微光——这一次，它没能阻止什么，只是默默地将你的记忆收好。然后，一切重归起点。下一条世界线，你将带着这一世的所有记忆，再次出发。'
            : cause === 'true_victory'
